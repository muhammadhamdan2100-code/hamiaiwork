...
const PRODUCTION_URL = "https://hamiaiworks.com";

/**
 * Resolves the canonical site URL with graceful fallback — never throws or
 * crashes if an environment variable is missing:
 *   1. NEXT_PUBLIC_SITE_URL — set this explicitly in any environment
 *      (Netlify, Railway, Render, self-hosted, etc.) to override.
 *   2. VERCEL_URL — automatically provided by Vercel on every preview and
 *      production deployment; used so preview URLs get correct canonical
 *      tags instead of all pointing at the production domain.
 *   3. Hardcoded production URL — the default if neither is set, so local
 *      dev and any host without env config still works out of the box.
 */
